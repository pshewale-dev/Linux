...
while read -r server; do
  ...
  # Attempt to connect to server over ssh
  if ssh -q -i /path/to/private/key -p 22 adm@"$server" exit &>/dev/null; then
    # Get last patch date
    last_patch=$(ssh -i /path/to/private/key -p 22 adm@"$server" "ls -al /var/lib/apt/periodic/update-success-stamp | awk '{print \$6, \$7, \$8}'")
    # Get uptime
